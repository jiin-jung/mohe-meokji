...
import com.mohemeokji.mohemeokji.dto.ExpiryPolicyDto;
...
public class IngredientShelfLifeService {
    ...
    public IngredientShelfLifeResponse getShelfLife(String ingredientName) {
        ...
        return new IngredientShelfLifeResponse(
                ingredientName,
                match.matchedName(),
                match.policy().shelfLifeDays(),
                toExpiryPolicy(match.policy())
        );
    }
    ...
    private ExpiryPolicyDto toExpiryPolicy(ShelfLifePolicy policy) {
        return new ExpiryPolicyDto(
                policy.trackExpiry(),
                policy.alertThresholdDays(),
                policy.displayMode(),
                policy.storageType()
        );
    }

    private void registerFresh(String ingredientName, int shelfLifeDays, String storageType, int alertThresholdDays) {
        policies.put(ingredientName, new ShelfLifePolicy(shelfLifeDays, storageType, true, alertThresholdDays, "FRESH"));
    }

    private void registerLongTerm(String ingredientName, int shelfLifeDays, String storageType, int alertThresholdDays) {
        policies.put(ingredientName, new ShelfLifePolicy(shelfLifeDays, storageType, true, alertThresholdDays, "LONG_TERM"));
    }

    private void registerOptional(String ingredientName, int shelfLifeDays, String storageType) {
        policies.put(ingredientName, new ShelfLifePolicy(shelfLifeDays, storageType, false, null, "OPTIONAL"));
    }
    ...
    private record ShelfLifePolicy(
            int shelfLifeDays,
            String storageType,
            boolean trackExpiry,
