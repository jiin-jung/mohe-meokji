...
public class YouTubeService {
    ...
    private boolean isRelevantRecipeVideo(JsonNode item, String recipeName, String searchQuery) {
        ...
        if (!StringUtils.hasText(collapsedText)) {
            return false;
        }

        String collapsedRecipeName = collapse(recipeName);
        if (StringUtils.hasText(collapsedRecipeName) && collapsedText.contains(collapsedRecipeName)) {
            return true;
        }

        List<String> significantTokens = buildSignificantTokens(recipeName, searchQuery);
        long matchedTokens = significantTokens.stream()
                .filter(collapsedText::contains)
                .count();

        boolean containsCookingHint = COOKING_HINTS.stream()
                .map(this::collapse)
                .anyMatch(collapsedText::contains);

        int requiredTokenMatches = Math.min(2, significantTokens.size());
        return requiredTokenMatches > 0 && matchedTokens >= requiredTokenMatches && containsCookingHint;
    }
    ...
    private void appendTokens(List<String> tokens, String source) {
        ...
        for (String token : source.split("\\s+")) {
            String collapsedToken = collapse(token);
            if (collapsedToken.length() >= 2 && !COOKING_HINTS.contains(token.toLowerCase(Locale.ROOT))) {
                tokens.add(collapsedToken);
            }
        }
    }
    ...
    private String collapse(String value) {
        ...
        return NON_ALPHANUMERIC_OR_HANGUL.matcher(value.toLowerCase(Locale.ROOT))
                .replaceAll("");
    }
}